...
with st.container():   
    ...
    try:
        ...
        path = '/Scopus Iteration'
        HtmlFile7 = open(r"/publicationJournalCocitation" + r".html", 'r')
    components.html(HtmlFile7.read(), height=700)
    try:
        df_html7 = pd.read_csv(r'/publicationJournalCocitation' + r'.html_communities.csv')
        # df_html2 = df_html2.reindex(sorted(df_html2.columns), axis=1)
...
with st.container():   
    ...
    try:
        ...
        HtmlFile7 = open(r"/publicationJournalBibliographiccoupling" + r".html", 'r')
    components.html(HtmlFile7.read(), height=700)
    try:
        df_html7 = pd.read_csv(r'/publicationJournalBibliographiccoupling' + r'.html_communities.csv')
        # df_html2 = df_html2.reindex(sorted(df_html2.columns), axis=1)
        df_html7.columns = ['cluster ' + str(col) for col in df_html7.columns]
        df_html7 = df_html7.dropna(how='all')
        df_html7 = df_html7.fillna('')
